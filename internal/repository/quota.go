...
func (s *sqlStore) ListTenantQuotas(ctx context.Context) ([]TenantQuota, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT tenant_id, max_bytes, max_objects, used_bytes, used_objects, updated_at FROM tenant_quotas`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&q.TenantID, &q.MaxBytes, &q.MaxObjects, &q.UsedBytes, &q.UsedObjects, &t); err != nil {
			return nil, err
		}
		q.UpdatedAt = t.Time
		out = append(out, q)
...
func (s *sqlStore) GetTenantQuota(ctx context.Context, tenant string) (TenantQuota, error) {
	...
	row := s.db.QueryRowContext(ctx, s.rebind(`SELECT tenant_id, max_bytes, max_objects, used_bytes, used_objects, updated_at FROM tenant_quotas WHERE tenant_id=$1`), tenant)
	var (
		q TenantQuota
		t flexTime
	)
	if err := row.Scan(&q.TenantID, &q.MaxBytes, &q.MaxObjects, &q.UsedBytes, &q.UsedObjects, &t); err != nil {
