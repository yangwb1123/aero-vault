CREATE TABLE event_outbox (
  id                  INTEGER PRIMARY KEY AUTOINCREMENT, -- authority dedupe key; AUTOINCREMENT so prune never reuses a row id
  event_type          TEXT NOT NULL CHECK (event_type IN
                        ('vault.file.deleted@1.1','vault.file.notify@1.1')),
  origin_id           INTEGER NOT NULL,                   -- objects.id reference; no FK (RestoreObject reuses row ids, reconcile prunes objects)
