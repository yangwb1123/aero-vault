...
// configEnvKeys contains every static environment key read by Load and its
// sub-loaders. Clearing the complete set prevents host values and a nearby
// .env file from changing which validation path a test exercises.
var configEnvKeys = []string{
	"ACCESS_CONTROL_ENABLED", "ACCESS_DEFAULT_POLICY", "ACCESS_DELETE_FAIL_CLOSED",
	"ACCESS_PUBLIC_BASE_URL", "ACCESS_SHARE_SECRET",
	"ADMIN_RATE_LIMIT_BURST", "ADMIN_RATE_LIMIT_RPS",
	"AI_AGENT_MAX_STEPS", "AI_CHAT_API_KEY", "AI_CHAT_ENDPOINT", "AI_CHAT_MODEL",
	"AI_CHAT_PROVIDER", "AI_CHUNK_OVERLAP", "AI_CHUNK_WINDOW",
	"AI_COST_COMPLETION_PER_1K", "AI_COST_PROMPT_PER_1K", "AI_DEGRADED_MODE",
	"AI_EMBED_API_KEY", "AI_EMBED_CACHE_SIZE", "AI_EMBED_DIM", "AI_EMBED_ENDPOINT",
	"AI_EMBED_MODEL", "AI_EMBED_PROVIDER", "AI_EXTRACTOR_API_KEY",
	"AI_EXTRACTOR_ENDPOINT", "AI_HYBRID_SEARCH", "AI_INDEX_ENABLED",
	"AI_LEXICAL_BACKEND", "AI_PER_TENANT_BUDGETS", "AI_PII_REDACT", "AI_PII_SCAN",
	"AI_RATE_LIMIT_BURST", "AI_RATE_LIMIT_RPS", "AI_REINDEX_STALE_ON_START",
	"AI_RERANK_API_KEY", "AI_RERANK_ENDPOINT", "AI_RERANK_MODEL", "AI_RERANK_PROVIDER",
	"AI_SEARCH_CACHE_SIZE", "AI_SEARCH_CACHE_TTL_SECONDS", "AI_TENANT_DAILY_BUDGET_USD",
	"AI_VECTOR_API_KEY", "AI_VECTOR_BACKEND", "AI_VECTOR_COLLECTION", "AI_VECTOR_DSN",
	"AI_VECTOR_URL", "APP_ADDR", "APP_IDLE_TIMEOUT", "APP_LOG_LEVEL",
	"APP_MAX_BODY_SIZE", "APP_TLS_CERT_FILE", "APP_TLS_ENABLED", "APP_TLS_KEY_FILE",
	"APP_WRITE_TIMEOUT", "AUDIT_GOVERNANCE_BASE_URL", "AUDIT_GOVERNANCE_BATCH_SIZE",
	"AUDIT_GOVERNANCE_BINDINGS_FILE", "AUDIT_GOVERNANCE_CLAIM_TTL_SECONDS",
	"AUDIT_GOVERNANCE_CLEANUP_BATCH_SIZE", "AUDIT_GOVERNANCE_CLEANUP_INTERVAL_SECONDS",
	"AUDIT_GOVERNANCE_DELIVERED_RETENTION_SECONDS", "AUDIT_GOVERNANCE_DRAIN",
	"AUDIT_GOVERNANCE_ENABLED", "AUDIT_GOVERNANCE_HMAC_KEY",
	"AUDIT_GOVERNANCE_HTTP_TIMEOUT_SECONDS", "AUDIT_GOVERNANCE_INITIAL_BACKOFF_SECONDS",
	"AUDIT_GOVERNANCE_MAX_BACKOFF_SECONDS", "AUDIT_GOVERNANCE_MAX_LAG_SECONDS",
	"AUDIT_GOVERNANCE_POLL_MILLISECONDS", "AUDIT_GOVERNANCE_RECONCILE_BATCH_SIZE",
	"AUDIT_GOVERNANCE_TOKEN_URL", "AUDIT_SINK_KIND", "AUDIT_SINK_L2_BINDINGS_FILE", "AUDIT_SINK_L2_ENDPOINT",
	"AUTH_ANONYMOUS_PUBLIC_READ", "AUTH_JWKS_AUDIENCE", "AUTH_JWKS_CLIENT_TENANTS",
	"AUTH_JWKS_DEFAULT_SCOPES", "AUTH_JWKS_ENDPOINT", "AUTH_JWKS_KEY_TTL",
	"AUTH_JWKS_TENANT_CLAIM", "AUTH_JWT_ISSUER", "AUTH_JWT_SECRET", "AUTH_KEYS",
	"AUTH_KEY_CACHE_TTL_SECONDS", "AUTH_OIDC_AUTHORIZATION_ENDPOINT", "AUTH_OIDC_CLIENT_ID",
	"AUTH_OIDC_ISSUER", "AUTH_OIDC_REDIRECT_URI", "AUTH_OIDC_SCOPES",
	"AUTH_OIDC_TOKEN_ENDPOINT", "AUTH_PERSIST_KEYS", "AUTH_PRESIGN_SECRET",
	"AV_API_KEY", "AV_ENABLED", "AV_ENDPOINT", "AV_PROVIDER", "AV_QUARANTINE",
	"BILLING_ALLOW_INSECURE_HTTP", "BILLING_BASE_URL", "BILLING_BINDINGS_FILE",
	"BILLING_ENABLED", "BILLING_HTTP_TIMEOUT_SECONDS", "BILLING_MAX_LAG_SECONDS",
	"BILLING_OUTBOX_BATCH_SIZE", "BILLING_OUTBOX_CLAIM_TTL_SECONDS",
	"BILLING_OUTBOX_MAX_ATTEMPTS", "BILLING_OUTBOX_POLL_MILLISECONDS",
	"BILLING_PROJECTION_INTERVAL_SECONDS", "BILLING_TOKEN_URL", "CORS_ALLOWED_HEADERS",
	"CORS_ALLOWED_METHODS", "CORS_ALLOWED_ORIGINS", "CORS_EXPOSE_HEADERS", "DB_DRIVER",
	"DB_DSN", "EVENTS_SUB_BUFFER", "EVENTS_TRANSPORT", "EVENTS_TRANSPORT_DSN",
	"EVENTS_WEBHOOK_SECRET", "EVENTS_WEBHOOK_URL", "EVENT_OUTBOX_BATCH_SIZE",
	"EVENT_OUTBOX_CLAIM_TTL_SECONDS", "EVENT_OUTBOX_DELIVERED_RETENTION_HOURS",
	"EVENT_OUTBOX_ENABLED", "EVENT_OUTBOX_FAILED_RETENTION_HOURS",
	"EVENT_OUTBOX_HTTP_TIMEOUT_SECONDS", "EVENT_OUTBOX_MAX_ATTEMPTS",
	"EVENT_OUTBOX_POLL_INTERVAL_MILLIS", "IDEMPOTENCY_HASH_BODY", "IDEMPOTENCY_TTL_HOURS",
	"JOBS_MAX_DEPTH", "JOBS_WORKERS", "MAX_INFLIGHT_REQUESTS", "PER_TENANT_CONCURRENCY_MAX",
	"PROMETHEUS_ENABLED", "RATE_LIMIT_BURST", "RATE_LIMIT_RPS",
	"RECONCILE_CLUSTER_SINGLETON", "RECONCILE_DELETE_ORPHAN_BLOBS",
	"RECONCILE_INTERVAL_MINUTES", "RECONCILE_ORPHAN_GRACE_MINUTES",
	"RECONCILE_RETENTION_DAYS", "RECONCILE_SCRUB_ENABLED", "RECONCILE_TENANTS",
	"REPLICATION_BACKEND", "REPLICATION_ENABLED", "REPLICATION_LOCAL_ROOT",
	"REPLICATION_LOCAL_SIGN_KEY", "REPLICATION_LOCAL_SSE_KEY", "REPLICATION_LOCAL_SSE_KEYFILE",
	"REPLICATION_RESYNC_INTERVAL_MINUTES", "REPLICATION_S3_ACCESS_KEY",
	"REPLICATION_S3_BUCKET", "REPLICATION_S3_ENDPOINT", "REPLICATION_S3_FORCE_PATH_STYLE",
	"REPLICATION_S3_REGION", "REPLICATION_S3_SECRET_KEY", "REQUEST_TIMEOUT_SECONDS",
	"S3_COMPAT_PREFIX", "S3_SIGV4_CREDENTIALS", "STORAGE_BACKEND", "STORAGE_CB_ENABLED",
	"STORAGE_CB_FAILURE_THRESHOLD", "STORAGE_CB_HALF_OPEN_MAX", "STORAGE_CB_RECOVERY_TIMEOUT",
	"STORAGE_CONNECT_TIMEOUT", "STORAGE_COS_BUCKET_URL", "STORAGE_COS_SECRET_ID",
	"STORAGE_COS_SECRET_KEY", "STORAGE_DEFAULT_CLASS", "STORAGE_LOCAL_PUBLIC_URL",
	"STORAGE_LOCAL_ROOT", "STORAGE_LOCAL_SIGN_KEY", "STORAGE_LOCAL_SSE_KEY",
	"STORAGE_LOCAL_SSE_KEYFILE", "STORAGE_LOCAL_SSE_KEY_TOKEN", "STORAGE_LOCAL_SSE_KEY_URL",
	"STORAGE_LOCAL_SSE_KMS_KEY_ID", "STORAGE_LOCAL_SSE_KMS_TOKEN", "STORAGE_LOCAL_SSE_KMS_URL",
	"STORAGE_OSS_ACCESS_KEY", "STORAGE_OSS_BUCKET", "STORAGE_OSS_ENDPOINT",
	"STORAGE_OSS_SECRET_KEY", "STORAGE_READ_TIMEOUT", "STORAGE_S3_ACCESS_KEY",
	"STORAGE_S3_BUCKET", "STORAGE_S3_ENDPOINT", "STORAGE_S3_FORCE_PATH_STYLE",
	"STORAGE_S3_REGION", "STORAGE_S3_SECRET_KEY", "STORAGE_SSE_REWRAP_ON_START",
	"STORAGE_VERIFY_MAX_SIZE", "STORAGE_VERIFY_ON_READ", "STORAGE_VERIFY_SAMPLE",
	"STORAGE_WRITE_TIMEOUT", "THUMBNAIL_CACHE_BYTES", "THUMBNAIL_CACHE_TTL",
	"THUMBNAIL_PER_TENANT_DECODE_SLOTS", "UPLOAD_GC_TTL_HOURS", "WEBDAV_PREFIX", "WEBUI_ENABLED",
}

// clearEnv isolates a configuration test from host and repository environment.
// t.Setenv restores original values at test end.
