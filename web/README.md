...
持久 SSE 对象动态与断线回放、存储桶创建/版本控制/对象锁/生命周期/ACL/Policy/CORS/SSE、对象标签/元数据/版本/Legal Hold、对象分享、公开图片、资源 ACL、部门与 aero-id 成员映射、
可移植备份、operator 任务/租户/Webhook 运维，以及 AI 搜索、SSE 流式对话、Agent 工具轨迹和对象使用血缘查询。
