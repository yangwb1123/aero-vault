...
> **去重验证：** 对 `docs/requirements/` 下全部 100 份既有分析文档（`expansion-directions.md` ~ `expansion-v100-architect-frontiers.md`）逐方向进行关键词正则 + 语义交叉验证 + 代码锚点校验  
> **日期：** 2026-07-11  
> **核心原则：** 不编写任何代码。选取**代码中存在明确锚点、有实质性产品/架构影响、且在 100 轮既有分析中未被独立深度覆盖**的方向。
