...
> **去重验证：** 对 `docs/requirements/` 下全部 111 份既有分析文档逐方向进行关键词正则 + 语义交叉验证 + 代码锚点反查，确认本文方向未被已有文档独立深度覆盖  
> **日期：** 2026-07-11  
...
AeroVault 已经是一个非常成熟的项目。大路货的扩展点（桶策略、CORS、通知、日志、版本控制、多租户、AI管线、ID基数等）在前 111 轮分析中已经被反复覆盖。
...
| # | 方向 | 类型 | 优先级 | 核心痛点 | 代码证据 |
|---|------|------|--------|---------|---------|
| **1** | **S3 ListObjects Delimiter/CommonPrefixes 缺失**——S3 兼容层不支持文件夹式层级浏览 | 协议兼容/产品 | **P0** | S3 ListObjectsV2 的 `delimiter` 参数被完全忽略。任何依赖 S3 虚拟文件夹访问模式的工具（aws-cli、rclone、Cyberduck、AWS SDK 的 `ListObjectsV2Paginator`）会收到扁平列表而非目录结构。文件无法按 `/` 分隔的层级组织浏览 | `internal/api/s3compat/handler.go:458-506` — `listObjectsV2` 解析 `prefix`/`continuation-token`/`max-keys`/`tag-key`/`tag-value`，但完全没有读取 `delimiter` 查询参数；`internal/repository/sql_objects.go:166-195` — `ListObjects` 的 SQL 查询仅 `WHERE key LIKE prefix%`，无 `delimiter` 分组逻辑；`internal/api/s3compat/xml.go:14-23` — `listBucketResult` 结构体定义了 `CommonPrefixes` 字段但没有被填充 |
| **2** | **S3 UploadPartCopy 缺失**——超过 5GB 的对象无法跨键复制 | 协议兼容 | **P0** | CopyObject 使用 `io.Copy` 流式读取整个源对象再写入目标。当对象 >5GB 时 AWS SDK 自动回退到 multipart copy（UploadPartCopy），但该 API 端点未实现，导致大对象复制必然失败 | `internal/api/s3compat/extra.go:33-65` — `copyObject` 使用 `h.svc.Get()` 读取整个源对象然后用 `h.svc.Put()` 写入；`internal/api/s3compat/handler.go:77-83` — `PutObject` 仅在检测到 `x-amz-copy-source` 时调用 `copyObject`，无 multipart copy 的 `?partNumber&uploadId` 分发路径；`internal/api/s3compat/handler.go:388-395` — `PostObject` 仅分发 `?uploads` 和 `?uploadId`，无 UploadPartCopy 路由 |
| **3** | **S3 服务端加密请求头盲区**——SSE-S3/SSE-KMS/SSE-C 请求头被静默忽略 | 协议兼容/安全 | **P1** | 主流 S3 SDK（boto3、aws-sdk-go、AWS CLI）默认发送 `x-amz-server-side-encryption` 请求头。当前实现在 `PutObject` 路径中完全不读取这些表头，既不存储也不返回。客户端以为启用了加密但实际未生效 | `internal/api/s3compat/handler.go:80-108` — `PutObject` 读取的请求头集合：`x-amz-copy-source`/`tagging`/`uploadId`/`acl`/`restore`/`Content-Type`/`Content-MD5`/`x-amz-storage-class`/`x-amz-acl`/`x-amz-object-lock-legal-hold`，但**没有任何 SSE 相关头**；`internal/service/file_crud.go:176-182` — `Put` 方法的 `PutOptions` 结构体没有 `SSEAlgorithm`/`SSEKMSKeyID`/`SSECustomerKey` 字段；`internal/repository/sql_objects.go:33-50` — `objects` 表的 INSERT 语句没有 `sse_algorithm`/`sse_kms_key_id` 列 |
| **4** | **存储分层生命周期转换缺失**——`storage_class` 被记录但不被转换 | 架构/成本 | **P1** | 对象模型已包含 `StorageClass` 字段，`x-amz-storage-class` 请求头被正确解析和持久化，遥测已按 storage class 分类统计。但生命周期仅支持过期删除（`expire_after_days` → `soft_delete`/`hard_delete`），无 STANDARD → STANDARD_IA → GLACIER 的自动分层转换 | `internal/repository/repository.go:41-44` — `BucketConfig` 只有 `ExpireAfterDays`/`ExpireAction`，无 `Transition` 数组；`internal/api/s3compat/xml.go:215-227` — `lifecycleConfiguration` 的 XML 模型只有 `Expiration` 没有 `Transition`；`internal/reconcile/lifecycle.go:16-90` — `LifecycleJob.sweepExpired` 仅执行过期删除，无任何分层转换逻辑；`internal/telemetry/metrics.go:181-183` — `RegisterStorageClassGauge` 已按 storage class 统计计数，但数据永不变化 |
| **5** | **对象锁合规模式盲区**——`GOVERNANCE` 与 `COMPLIANCE` 无区分，`BypassGovernance` 未实现 | 合规/安全 | **P2** | ObjectLockConfiguration 可以被设置（`PUT ?object-lock`），`locked_until` 被正确执行（阻止删除/覆盖）。但系统不区分 GOVERNANCE（可被特权用户绕过）和 COMPLIANCE（不可绕过）两种保留模式，也不支持 `x-amz-bypass-governance-retention` 请求头和 `s3:BypassGovernanceRetention` 权限评估 | `internal/service/file_crud.go:295-310` — `deleteObject` 检查 `locked_until` 但只检查 `obj.LockedUntil != nil && obj.LockedUntil.After(time.Now())`，不区分 GOVERNANCE/COMPLIANCE；`internal/api/s3compat/handler.go:90-99` — `PutObject` 将 `x-amz-object-lock-legal-hold: ON` 存入 `_aero_legal_hold` metadata，但无 `PUT ?legal-hold` 独立端点；`internal/api/s3compat/bucketconfig.go:171-200` — `getBucketObjectLock`/`putBucketObjectLock` 仅处理 `ObjectLockSeconds`，不持久化 `RetentionMode`；`internal/repository/repository.go:35-40` — `BucketConfig.ObjectLockSeconds` 是 `int` 而非结构化保留配置 |
...
```go
// internal/api/s3compat/handler.go:458-506 — listObjectsV2
...
```go
// internal/api/s3compat/xml.go:14-23 — listBucketResult 结构体
type listBucketResult struct {
    // ✅ CommonPrefixes 已定义但从未被填充
...
```go
// internal/api/s3compat/handler.go:388-395 — PostObject 无 UploadPartCopy 路由
func (h *Handler) PostObject(w http.ResponseWriter, r *http.Request) {
    ...
    // ❌ 无 UploadPartCopy 分支 (PUT with ?partNumber&uploadId + x-amz-copy-source)
    default:
...
```go
// internal/api/s3compat/handler.go:80-108 — PutObject 请求头读取
...
```go
// internal/api/s3compat/handler.go:148 — 响应头设置
writeObjectHeaders(w, obj.ContentType, obj.Size, obj.ETag,
    obj.UpdatedAt.Format(http.TimeFormat), obj.StorageClass, obj.Metadata)
// ❌ 无 x-amz-server-side-encryption 响应头
```
...
```go
// internal/api/s3compat/xml.go:215-227 — Lifecycle XML 模型
...
func (s *FileService) deleteObject(ctx context.Context, tenant, bucket, key string, version string) error {
    // ...
    if obj.LockedUntil != nil && obj.LockedUntil.After(time.Now()) {
        return fmt.Errorf("%w: object is locked until %s", ErrLocked, obj.LockedUntil.Format(time.RFC3339))
    }
...
func (h *Handler) putBucketObjectLock(w http.ResponseWriter, r *http.Request, bucket string) {
    // 解析 XML body 但只提取 Days，丢弃 Mode
    seconds = in.Rule.DefaultRetention.Days * 86400
    // ❌ in.Rule.DefaultRetention.Mode 被忽略
}
```
...
3. **`retention_mode` 在 BucketConfig 中的持久化** — 当前 `ObjectLockSeconds int` 扩展为结构化配置
...
*本文档基于全代码库扫描，聚焦于第 112 轮分析中尚未被独立深度覆盖的 5 个高价值方向。每个方向均附有精确的代码锚点、生产影响分析和架构权衡。*
