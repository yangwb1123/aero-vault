...
| C8 | Postgres 方言 | 审计 INSERT 无方言差异（占位符走 `s.rebind`）；无需 0042 迁移（`payload` 为 TEXT，schema 演进在应用层） |
| C9 | **接收方幂等预期**（契约评审阻塞项 1） | 重复 POST 是设计内行为（租约重 claim / 崩溃重投 / F10）；接收方**必须幂等**（F3）：去重键 = `X-Audit-Fact-Id`（事实 id，每事实唯一，echo receipt 携带）+ 载荷字节 SHA-256（跨版本兜底）；重复 audit 行**可接受且预期内**；`object_id` 不可作去重键（C1） |
