...
| **5** | **S3 Select 服务端查询引擎** | 协议兼容 / 性能 | **P3** | S3 `SelectObjectContent` API（服务端对 CSV、JSON、Parquet 对象执行 SQL 查询并只返回结果子集）完全缺失。当前用户必须下载整个对象才能在客户端解析，带宽浪费严重。缺少此 API 导致 Spark、Presto、Athena 等大数据工具无法高效使用 aero-vault | **❌ 零覆盖**（代码 `grep -rn "SelectObjectContent\|selectObjectContent\|select-type\|s3select"` → 0 命中；117 份既有文档 → 0 实质性分析） |
