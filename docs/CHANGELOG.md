...
### Changed
- **Thumbnail: TTL-expired cache reads distinguished from genuine misses in hit-ratio metrics** (`internal/thumbnail/cache.go`, `internal/thumbnail/cache_entry.go`, `internal/telemetry/thumbnail.go`, `internal/telemetry/metrics.go`)
  - `Cache.Get` now returns a tri-state outcome (`GetHit`/`GetMiss`/`GetExpired`): a TTL-expired read is removed and classified as its own class — **not** a miss — so `Cache.Stats` (now `hits, misses, evictions, expired`) and the new `thumbnail_cache_expired_total` counter (dotted `thumbnail.cache.expired_total`) account for it separately. `lookupCached` routes the outcome to `IncThumbnailCacheExpired`; an expired read still returns `(nil, false)` upstream, so the miss body runs and re-stores with a fresh expiry — served behavior is byte-identical.
  - **Semantics change to `thumbnail_cache_misses_total`:** it now counts genuine misses only (in-repo consumers — the Grafana panel and `ThumbnailCacheHitRatioLow` — are description-updated; external dashboards consuming the miss series are out of the repo's control). The hit-ratio expression is unchanged: an all-expired workload reads hits=0 ∧ misses=0, so the activity guard keeps the alert silent (previously it fired permanently on a healthy sparse-TTL cache) and the panel shows no data while `expired_total` rises — triage via `rate(thumbnail_cache_expired_total[5m])` and size `THUMBNAIL_CACHE_TTL` above the hot-key inter-request gap. Sweep removals remain not-reads (`SweepExpired` touches none of the four counters). This is monitoring telemetry (CC7.3 observability); per-event audit evidence for the retention control belongs to the platform L0/L1/L2 audit hierarchy and is a documented follow-up, not part of this change. No new config, no migration, no `go.mod` change (the `expired_total` registration lives with the moved `initThumbnailInstruments` helper, keeping `metrics.go` at 491 lines).
...
### Fixed
- **Thumbnail: 200-path `Last-Modified` derives from the object version actually decoded; 304 arm's duplicated ETag/Last-Modified sets removed** (`internal/api/rest/thumbnail.go`, `internal/api/rest/thumbnail_test.go`, `internal/api/rest/thumbnail_cache_test.go`)
  - Under a concurrent PUT between the pre-open Stat and the Get open, the 200 previously paired the new version's `ETag`/`X-Version-Id` with the old version's mtime — a mixed-version validator triplet that could drive an If-Modified-Since-only revalidating cache to a stale 304 while the served entity had changed (RFC 9110 §8.8.3: Last-Modified must be the date of the selected representation). The 200 `Last-Modified` now comes from the opened object when one exists — the same capture-then-serve lifecycle as the opened-derived ETag/X-Version-Id; the `opened == nil` guard keeps cache-hit responses Stat-derived (the cache key derives from that Stat's ETag, so its mtime is the coherent freshness basis). Pinned requests are a strict no-op (immutable version row ⇒ `opened.UpdatedAt == obj.UpdatedAt`).
  - The 304 arm's first `ETag`/`Last-Modified` pair was dead code — the second byte-identical set always won; removed, so each header is now set exactly once per 304. Stable-object 200s and all 304s are byte-identical. New `TestThumbnailLastModifiedDerivedFromOpenedObject` (+ `application/octet-stream` sniff-bucket variant) pin the miss path and fail red pre-fix; `TestThumbnailCacheHitLastModifiedIsStatDerived` pins the hit path. No new config, no `openapi.json` change.
...
### Added
- **Thumbnail: depth-16 PNG sources capped at `Max16BitSourceDim` 4096** (`internal/thumbnail/png16.go`, `internal/thumbnail/thumbnail.go`, `internal/thumbnail/thumbnail_test.go`, `internal/thumbnail/large_transparent_allocation_test.go`, `internal/thumbnail/semaphore_test.go`)
  - stdlib `image/png` decodes depth-16 sources (color types 2/4/6) to `*image.RGBA64`/`*image.NRGBA64` at **8 B/px**, so a depth-16 PNG at `MaxSourceDim` (8192²) reached ≈ 544 MiB live per request and ≈ 2.125 GiB aggregate across the 4 decode slots — a documented breach of the 2 GiB ceiling pinned in `semaphore_test.go`. Depth-16 sources with any side above **4096** are now rejected from the header (`pngBitDepth` reads the IHDR bit-depth byte at offset 24 from the already-buffered DecodeConfig tee, ≤ 33 bytes consumed) with the existing `ErrImageTooLarge` → 413, before any pixel buffer — mirroring the `MaxProgressiveSourceDim` precedent. Worst case drops to 128 MiB decode / ≈ 160 MiB live; the depth-16 aggregate (≈ 4 × 128 MiB ≈ 512 MiB) is back inside the pinned ceiling. The cap is format-class (bit depth 16 regardless of color type; gray-only 16-bit, 2 B/px, is conservatively included); ≤ 4096² depth-16 sources, all 8-bit PNGs (including 8192²), GIFs and all JPEG paths are byte-identical in behavior and output. Compatibility note: previously-valid 8192² depth-16 sources now return 413.
- **Thumbnail REST: 415 for unsupported image types; 400 for invalid `?w=`/`?h=`** (`internal/thumbnail/errors.go`, `internal/api/rest/thumbnail.go`, `internal/api/rest/handler_helpers.go`, `internal/api/rest/specgen.go`, `internal/api/rest/router.go`)
  - `GET /v1/files/{key}/thumbnail` now returns **415 UnsupportedMediaType** for `image/*` content types outside the decodable set, via the new sentinel `thumbnail.ErrUnsupportedFormat`, with the supported-type list in the message. The gate is **mime-normalized per RFC 9110 §8.3.1** (`mime.ParseMediaType`): media types are case-insensitive and parameters are stripped, so `Image/JPEG` and `image/jpeg; charset=utf-8` normalize to `image/jpeg` and keep working (200), while true aliases of unsupported formats (`image/jpg`, `image/x-png`, `image/pjpeg`, webp/bmp/avif/tiff) are 415 — a valid image the server cannot decode is a capability matter (RFC 9110 §15.5.16), not a client argument error. Previously these surfaced as 400 `InvalidArgument`. Non-image content types (and unparseable media types) keep the existing 400 "object is not an image" path; corrupt bytes of a whitelisted type stay 400 via the unchanged byte-level `ErrUnsupported`.
...
### Changed
- **Thumbnail REST: 200-path ETag derived from the Get-opened object, closing the Stat→Get TOCTOU** (`internal/api/rest/thumbnail.go`, `internal/api/rest/thumbnail_test.go`)
  - The 200-path validator previously came from the pre-open `Stat` row while the body decoded the bytes of the object opened *after* it: a concurrent PUT/POST landing between the two reads paired a version-N validator with version-N+1 bytes, and shared caches (anonymous path is `public, max-age=86400`) could retain the mismatch for up to 24 h under a matching `If-None-Match` 304. The 200 ETag is now derived from the `repository.Object` returned by the opener's `svc.Get` — the row whose ETag describes the bytes actually decoded (`fmt.Sprintf("%s-thumb-%dx%d", opened.ETag, effW, effH)`), mirroring the Get handler's capture-then-serve pattern (`handler.go` → `handleRangeOrFull`). The pre-open Stat validator still serves only the 304 fast path (unchanged; short-circuits before the stream opens, pinned by a repo-seam GetObject-count assertion). Stable-object responses are byte-identical (opened ETag == Stat ETag); a defensive fallback keeps the Stat validator if no opened object were captured (unreachable under the current API contract). No new status, header, config, or `openapi.json` change; existing 304/cache-fragmentation guarantees unaffected.
